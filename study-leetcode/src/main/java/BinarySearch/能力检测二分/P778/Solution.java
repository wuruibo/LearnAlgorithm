package BinarySearch.能力检测二分.P778;
...
public class Solution {
    ...
    public int swimInWater(int[][] grid) {
        ...
        while (left<=right){
            ...
            if(bfs(grid,new boolean[n][n],mid)){
                right=mid-1;
            }else {
                left=mid+1;
            }
    ...
    private boolean bfs(int[][] grid,boolean[][] visited,int k){
        ...
        while (!queue.isEmpty()){
            ...
            //1.收集当前元素四个方向上符合条件的元素加入队列中
            for (int[] dir : directions) {
                int nx=dir[0]+x;
                int ny=dir[1]+y;
                if (inOrder(grid,nx,ny) && !visited[nx][ny] && grid[nx][ny]<=k) {

...
class Solution {
    ...
    private int bfs(int[][] land,int[] position){
        Queue<int[]> queue= new LinkedList<>();
        queue.offer(position);
        //将当前遍历到的设置为-1 标记一下
        land[position[0]][position[1]]=-1;
        int result=0;
        while (!queue.isEmpty()) {
            ...
            for (int[] direction : directions) {
                ...
                if (isValid(next,land) && land[next[0]][next[1]]==0) {
                    queue.offer(next);
                    land[next[0]][next[1]]=-1;
                }
            }
        }
        return result;
    }
    private boolean isValid(int[] position,int[][] land){
        return position[0]<land.length && position[0]>=0 && position[1]<land[0].length && position[1]>=0;
    }

    public static void main(String[] args) {
        System.out.println(new Solution().pondSizes(new int[][]{{2,8,8,4,1,7,1,5,4,2,0,8,1,9,2,5,6,9,7,4,8,9,1,7,3,6,9,3,7,1,5,7,3,1,9,9,9,6,5,1,9,9,4,1,6,4,7,6,1,6,1,3,3,9,0,3,0,0,0,1,3,1,2,0,0,0,3,2,7,3,6,7,9},
